...
angular.module('WebApp').controller('JobController' , ['$state', '$stateParams', '$rootScope', '$scope','$filter', 'settings','ListBucket','Bucket','ListAWSCredential','ListServerCredential','ListJob','RDBMS','TwiiterKafka','Kafka','ML','Workflow', function( $state, $stateParams ,$rootScope, $scope,$filter,settings,ListBucket,Bucket,ListAWSCredential,ListServerCredential,ListJob,RDBMS,TwiiterKafka,Kafka,ML,Workflow ) {
	...
	{
		ListJob.searchJobType($scope.userId).then(function (data) {
		...
		ListServerCredential.searchServerCredentials($scope.userId).then(function (data) {
             $rootScope.buildjson_ServerCredentials_data=data.serverCredential;
              $.each($rootScope.buildjson_ServerCredentials_data, function (index, valueobj) {
            	 if(valueobj.servertypeid != "4" && valueobj.servertypeid != "1")
             	  {
	...
	{  
 		...
 		TwiiterKafka.searchTwitterKafkaTopics($scope.userId).then(function (data) {
 			$('select[name="kafkatopicname"]').html('');
	...
	$scope.StopTwittersteaming= function()
	{
		Kafka.StopTwittersteaming().then(function (data) {
			 console.log(data);
			},function (errorMessage) {
	...
	$scope.upload = function()
    {
       ...
       if($scope.selectedSourceItem == null  && $scope.selectedDestinationItem != null)
       {
       	swal(
				    'Error!',
				    'Please select Source Server Item.',
				    'question'
				  )
				  return;
       }
        if($scope.selectedSourceItem !=null  && $scope.selectedDestinationItem ==null)
       {
       	swal(
				    'Error!',
				    'Please select Destination Server Item.',
				    'question'
				  )
				  return;
       }
        if($scope.selectedSourceItem ==null  && $scope.selectedDestinationItem ==null)
       {
       	swal(
				    'Error!',
				    'Please select Source & Destination Server Item.',
				    'question'
				  )
				  return;
       }
       ...
       if( $("input[name=hasgtag]").val() == '' && parseInt($scope.SourceServerType)  == 4)
       {
    		swal(
   				  'Error !',
   				  'Please enter Twitter Hasg Tag.',
   				  'question'
   				)
   			 return;
       	 
       }
       if( $("input[name=twitterbatch]").val()  == '' && parseInt($scope.SourceServerType)  == 4)
       {
    		swal(
   				  'Error !',
   				  'Please enter Twitter Batch Size.',
   				  'question'
   				)
   			 return;
    	...
    	if(  $scope.selectedSourceItem != null  && $scope.selectedDestinationItem != null )
        {
    		$scope.savejobJson=[{
    			...
    			jobType:  parseInt($scope.JobType), 
     			parentId:parseInt($scope.userId),
    	...
    	/*	else if($scope.DestinationServerType == 2 && $scope.JobType == 4)
    		...
    		{
    			$scope.savejobJson[0].sourcerhashtag=  $("input[name=hasgtag]").val();
    			$scope.savejobJson[0].sourcerbatchsize=	$("input[name=twitterbatch]").val();
    			$scope.savejobJson[0].sourcerkafkaName=	$('select[name="kafkatopicname"]').val();
    			if(typeof $scope.selectedDestinationItem.path.split('|')[1] == 'undefined')
	    			{
    		    ...
    		    $scope.savejobJson[0].destinationfileKey = DestinationkeyNew;	
    		}
    		...
    		if(  $scope.JobType  == 6 )
    		{
    	    		...
    	    		if(typeof $scope.selectedDestinationItem.path.split('|')[1] == 'undefined')
	    			{
	    				var filetrainName= "Train_"+Date.now()+'.zip';
 	    			}
	    			else
	    			{
	    				var filetrainName= $scope.selectedDestinationItem.path.split('|')[1]+$scope.selectedDestinationItem.name+'/'+"Train_"+Date.now()+'.zip' ;
	    			}
    	    		$scope.savejobJson[0].destinationfileKey = filetrainName;	
    			     
    		}
    		
    	    if($scope.JobType  == 4  || $scope.JobType  == 5)
    	    {
    	    	    
     	    		$scope.savejobJson[0].targetId=$scope.SourceId;
     	    		$scope.savejobJson[0].targetType= parseInt($scope.SourceServerType);
    			     ...
    			     else
    			     {
    			    	 ...
    			    	 $(MLVariableInDependents).each(function(index, brand){
    	    		         ...
    	    		         var trainkey= $scope.selectedTainBucket.Key.split(',');
	    	   				 var TrainSourcebucket=$scope.selectedTainBucket.path.split('|')[0];  
	    	   				 if(typeof $scope.selectedTainBucket.path.split('|')[1] == 'undefined'  || typeof $scope.selectedTainBucket.path.split('|')[1] == '')
	    	   	    		  	{
	    	   	    				var TrainSourcekey= $scope.selectedTainBucket.name;
	    	   	    			}
	    	   	    			else
	    	   	    			{
	    	   	    				var TrainSourcekey= $scope.selectedTainBucket.path.split('|')[1]+$scope.selectedTainBucket.name;
	    	   	    			}
	    	   					
	    	   				 
	    	   			 		$scope.savejobJson[0].trainaccessKey = trainkey[0];
	    	   			 		$scope.savejobJson[0].trainsecretKey = trainkey[1];
	    	   			 		$scope.savejobJson[0].trainbucketName = TrainSourcebucket;
	    	   			 		$scope.savejobJson[0].trainKey = TrainSourcekey;
 
    			     }
    			     
    			     
    			     if(    $scope.GetAllDestinationJson['classname'] == 'com.amazon.redshift.jdbc4.Driver')
    		         {
			            	...
			            	$scope.getBucketPropertiesJson=JSON.parse( $scope.getBucketPropertiesJson); 
			         		...
			         		bucketLocation=	data.bucketLocation;
			         			...
			         			$scope.savejobJson[0].dumpaccessKey = Keys[0];
	   			 				$scope.savejobJson[0].dumpsecretKey = Keys[1];
	   			 				$scope.savejobJson[0].dumpbucketName = $scope.selectedBucketDump.name;
	   			 				$scope.savejobJson[0].dumpKey = fileName;
	   			 				$scope.savejobJson[0].dumpbucketlocation = bucketLocation; 
     	   ...
     	   if( $scope.JobType  != 3  &&  $scope.JobType  != 7)
     		   {
     	  ...
     	  if($scope.formData.createNewTable == "1")
     	   ...
     	   if( $scope.JobType  == 7 )
 		   {
     		  ...
     		  $scope.savejobJson[0].sorcebucketName= Sourcebucket;
  			  $scope.savejobJson[0].sourcefileKey = Sourcekey;
  			  var dburlstring=$scope.GetAllDestinationJson['dburl'].split(':');
  			  dburlstring= dburlstring[2].split('//')[1]; 			
  			  $scope.savejobJson='{"bucketName":"'+Sourcebucket+'","accesskey":"'+$scope.GetAllSourceJson['accesskey']+'","secretKey":"'+$scope.GetAllSourceJson['secretKey']+'","dbname":"'+ $scope.GetAllDestinationJson['dbname']+'","dbpassword":"'+$scope.GetAllDestinationJson['dbpassword']+'","dbusername":"'+ $scope.GetAllDestinationJson['dbusername']+'","tablename":"'+$scope.savejobJson[0].destinationrdbmstableName+'","key":"'+Sourcekey+'","dburl":"'+dburlstring+'"}'
  			  $scope.savejobJson=JSON.parse( $scope.savejobJson);
  			  
  			   ML.Validate($scope.savejobJson).then(function (data) {
	  				//console.log(data);
	     			 setTimeout(function(){   
	     				 //$rootScope.buildjson_Job_data=  data[0];
		     			 swal("Success!", "Your job is added.", "success");
	    	...
	    	if(  $scope.selectedSourceItem != null  && $scope.selectedDestinationItem != null )
	        {
	    		...
	    		else if($scope.DestinationServerType == 2  )
	    		{
	    			...
	    			else
	    			{
	    		    	...
	    		    	var	overWriteTableData= false;
	    		    	$scope.savejobJson[0].destinationrdbmstableName =   $scope.selectedDestinationItem.name;
	    				$scope.savejobJson[0].destinationrdbmsnewtableTruncate = 1;
	    				if($scope.formData.overWriteTableData == "1")
		    			{
	    	    ...
	    	    //jobId
		    	    ...
		    	    //Filter json on base of job type 
		    	    if($scope.JobType == 1)
		    	    {
		    	    	//Ftp to s3
		    	    	if( $scope.SourceServerType == 1 && $scope.DestinationServerType == 3  )
			    		{
			    			$scope.FtpToS3UploadJson='{"api":"Syra/api/ftp/ftptos3upload","ftphostName":"'+$scope.GetAllSourceJson['ftphostName']+'","ftpPort":"'+$scope.GetAllSourceJson['ftpPort']+'","ftpUsername":"'+$scope.GetAllSourceJson['ftpUsername']+'","ftppassword":"'+$scope.GetAllSourceJson['ftppassword']+'","ftpPath":"'+$scope.savejobJson[0].sourcefolderPath+'","accesskey":"'+$scope.GetAllDestinationJson['accesskey']+'","secretKey":"'+$scope.GetAllDestinationJson['secretKey']+'","bucketName":"'+$scope.savejobJson[0].destinationbucketName+'","key":"'+$scope.savejobJson[0].destinationfileKey+'"}';
			    			$scope.savejobJson[0].jobjson=$scope.FtpToS3UploadJson;
				        	  
			    		}
		    	    	//ftp to mysql
		    	    	else if( $scope.SourceServerType == 1 && $scope.DestinationServerType == 2  )
		    	    	{
		    	    		$scope.FtpToMysqlJson='{"api":"Syra/api/sourcetodestination/ftptodatabase","ftphostName":"'+$scope.GetAllSourceJson['ftphostName']+'","ftpPort":"'+$scope.GetAllSourceJson['ftpPort']+'","ftpUsername":"'+$scope.GetAllSourceJson['ftpUsername']+'","ftppassword":"'+$scope.GetAllSourceJson['ftppassword']+'","ftpPath":"'+$scope.savejobJson[0].sourcefolderPath+'","ftpfileName":"'+$scope.savejobJson[0].sourcefileKey+'","classname": "'+ $scope.GetAllDestinationJson['classname']+'","dburl":"'+$scope.GetAllDestinationJson['dburl']+'","dbport":"'+ $scope.GetAllDestinationJson['dbport']+'","dbusername":"'+ $scope.GetAllDestinationJson['dbusername']+'","dbpassword":"'+$scope.GetAllDestinationJson['dbpassword']+'","dbname":"'+ $scope.GetAllDestinationJson['dbname']+'","createnewtable":"'+createNewTable+'","tableoverride":"'+ overWriteTableData+'","tablename":"'+$scope.savejobJson[0].destinationrdbmstableName+'","fieldseparator":","}';
			    			$scope.savejobJson[0].jobjson=$scope.FtpToMysqlJson;
		    	    	}
		    	    	//s3 to mysql n redshift
		    	    	else if(  $scope.SourceServerType == 3 && $scope.DestinationServerType == 2   )
	   					{
		    	    		//for s3 to redshift
		    	    		if( $scope.GetAllDestinationJson['databaseType'] == "Redshift" )
		   					{
	   			            	$scope.getBucketPropertiesJson='{"accesskey": "'+$scope.GetAllSourceJson['accesskey']+'", "secretKey": "'+$scope.GetAllSourceJson['secretKey']+'","bucketName": "'+$scope.savejobJson[0].sorcebucketName+'" }'; 
	    			            var bucketLocation='us-east-1';
	   			            	Bucket.getBucketProperties($scope.getBucketPropertiesJson).then(function (data) {
	   			         		...
	   			         		$scope.S3ToRedShiftJson='{"api":"Syra/api/sourcetodestination/redshiftdataload","accesskey":"'+$scope.GetAllSourceJson['accesskey']+'","secretKey":"'+$scope.GetAllSourceJson['secretKey']+'","bucketName":"'+$scope.savejobJson[0].sorcebucketName+'","key":"'+$scope.savejobJson[0].sourcefileKey+'","classname": "'+ $scope.GetAllDestinationJson['classname']+'","dburl":"'+$scope.GetAllDestinationJson['dburl']+'","dbport":"'+ $scope.GetAllDestinationJson['dbport']+'","dbusername":"'+ $scope.GetAllDestinationJson['dbusername']+'","dbpassword":"'+$scope.GetAllDestinationJson['dbpassword']+'","dbname":"'+ $scope.GetAllDestinationJson['dbname']+'","createnewtable":"'+createNewTable+'","tableoverride":"'+ overWriteTableData+'","tablename":"'+$scope.savejobJson[0].destinationrdbmstableName+'","fieldseparator":",","s3region":"'+bucketLocation+'"}';
		    	    		...
		    	    		{
		    	    			$scope.S3ToMysqlJson='{"api":"Syra/api/sourcetodestination/s3todatabase","accesskey":"'+$scope.GetAllSourceJson['accesskey']+'","secretKey":"'+$scope.GetAllSourceJson['secretKey']+'","bucketName":"'+$scope.savejobJson[0].sorcebucketName+'","key":"'+$scope.savejobJson[0].sourcefileKey+'","classname": "'+ $scope.GetAllDestinationJson['classname']+'","dburl":"'+$scope.GetAllDestinationJson['dburl']+'","dbport":"'+ $scope.GetAllDestinationJson['dbport']+'","dbusername":"'+ $scope.GetAllDestinationJson['dbusername']+'","dbpassword":"'+$scope.GetAllDestinationJson['dbpassword']+'","dbname":"'+ $scope.GetAllDestinationJson['dbname']+'","createnewtable":"'+createNewTable+'","tableoverride":"'+ overWriteTableData+'","tablename":"'+$scope.savejobJson[0].destinationrdbmstableName+'","fieldseparator":","}';
		   		    			$scope.savejobJson[0].jobjson=$scope.S3ToMysqlJson;
	  	    	    		}
		    	    		
	   					}	
		    	    	//Job Twitter
		    	    	else if(  $scope.SourceServerType == 4 && $scope.DestinationServerType == 3 )
	   					{
							...
							$scope.TwittersteamingJson='{  "api":"Syra/api/streaming/kafka/twitterstream" ,"consumerKey" :"'+$scope.GetAllSourceJson['consumerKey']+'","consumerSecret" : "'+$scope.GetAllSourceJson['consumersecret']+'","accessToken" : "'+$scope.GetAllSourceJson['accessToken']+'","accessTokenSecret" : "'+$scope.GetAllSourceJson['accessTokenSecret']+'","keyword" : "'+hasgtag+'", "topicName": "'+kafkatopicname+'","batchSize" : "'+twitterbatch+'","accesskey" : "'+$scope.GetAllDestinationJson['accesskey']+'","secretKey" : "'+$scope.GetAllDestinationJson['secretKey']+'","bucketName" : "'+$scope.savejobJson[0].destinationbucketName+'","key" : "'+DestinationkeyNew+'" } ';
		    	    ...
		    	    ListJob.saveJob($scope.savejobJson).then(function (data) {
	     				 ...
	     				 $state.go('job', {},{reload: true});
    	     	  		 ...
    	     	  		 swal("Error!", "Something went wrong!", "question");
   	     	 		});
	...
	$scope.sourcediv=true;
	$scope.sourceClass='col-md-6';
	$scope.destinationdiv=true;
	$scope.destinationClass='col-md-6';
	...
	$scope.jobtypeCheckedValue = function(JobTypeChecked)
	{
		   ...
		   // $('#destinationresult').removeClass('col-md-12').addClass('col-md-6');
		   ...
		   // $('#sourcediv').show();
            ...
            var i=0;
            var filtered=$filter('filter')($rootScope.buildjson_ServerCredentials_data, { servertypeid: 3},true);
             
            $.each( filtered, function (index, valueobj) {
            	var obj = {
                        id: valueobj.id,
                        name: valueobj.serverName,
                        object:'Dump AWS' ,
		...
		else if( JobTypeChecked == 4  ||  JobTypeChecked == 5)
		{

			$scope.sourcediv=true;
        	$scope.sourceClass='col-md-12';
 
        	$scope.destinationdiv=false;
        	$scope.destinationClass='col-md-6';
        	
   
			 
		    $scope.selectedSourceItem = 'table';
		    $scope.selectedDestinationItem = 'table';
		    $scope.rdbmsServerType=0;
			$scope.newTableName=0;
 		    $scope.MLJobType= 0;
 		    $scope.twittersource=0; 
   ...
   $scope.CustomCallbackTrain = function(item, selectedItems) {
    ...
    $scope.CustomCallbackSource = function(item, selectedItems,dataSourceObject)
    {
    	...
    	else if($scope.JobType == 4 ||  $scope.JobType == 5)
    	{
    ...
    $(document).on('change', 'select[name="source"]', function(){
    	...
    	if( $scope.JobType == 1   ||    $scope.JobType == 2    ||  $scope.JobType == 3   || $scope.JobType == 6)
 		{
 			$('#sourcediv').removeClass('col-md-12').addClass('col-md-6');
 		    $('#sourceresult').removeClass('col-md-12').addClass('col-md-6');
					...
					$scope.GetAllSourceJson=JSON.parse( $scope.GetAllSourceJson); 
 					RDBMS.AllTables($scope.GetAllSourceJson).then(function (data) {
						if(data.status)
						{
 						 ...
 						 if( $scope.JobType == 4 || $scope.JobType == 5)
 						 {
 							$scope.DestinationId = $scope.SourceId;
